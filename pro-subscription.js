...
(function () {
  ...
  // Dynamically load Razorpay if not yet available (ad-blocker/firewall may
  // have blocked the checkout.js <script> tag from loading on page load)
  function _ensureRazorpay() {
    return new Promise(function (resolve) {
      if (typeof Razorpay !== 'undefined') { resolve(true); return; }
      var s = document.createElement('script');
      s.src = 'https://checkout.razorpay.com/v1/checkout.js';
      s.onload  = function () { resolve(typeof Razorpay !== 'undefined'); };
      s.onerror = function () { resolve(false); };
      document.head.appendChild(s);
      setTimeout(function () { resolve(typeof Razorpay !== 'undefined'); }, 4000);
    });
  }
  ...
  window.initiateProSubscription = async function (toolName) {
    ...
    if (typeof Razorpay === 'undefined') {
      _toast('Loading payment…', '#6366F1');
      var rzpLoaded = await _ensureRazorpay();
      if (!rzpLoaded) {
        _toast('Payment blocked by your browser/network. Disable ad-blocker and retry, or email fino.sutra07@gmail.com.', '#EF4444');
        return;
      }
    }
    ...
    try {
      ...
      _toast('Network error, or payment was blocked. Please try again, or email fino.sutra07@gmail.com.', '#EF4444');
      console.error(err);
    }
  };
